...
| `chat` | vivim.chat | `conv:*` (conversation rows, latest-wins), `msg:*` (messages, append-only ledger; D-358: `{id, conversationId, role, seq, content, providerId?, realizationRef?, streamRef?, createdAt}` — realizationRef = producing realization, streamRef = M1's chunk envelope when streamed) | chat.open (conversations), chat.append (messages); chat.history reads; chat.resolve writes ns `resolve`, never ns `chat` verdicts twice | Conversations accumulate; per D-335 the retention policy stays OPEN with a named revisit trigger — message latency or compaction pressure in benchmarks, not before; the CHAT_HISTORY_CAP scan bound refuses at cap (fail-closed, never silent truncation) |
