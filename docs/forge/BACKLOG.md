...
- **A3 · the invariants round (pass #2 + the freshness stage)** — **NEXT tooling round** (the review's build-order row 4, still executable without the S3 call): regenerate `CURRENT-INVARIANTS.md` from the decision records (fold in Wave 0's forge constitution, the Core Phase seams, the generated-row era, the D-391 B5 re-freeze) + the report-only `invariants-freshness` gate stage (staleness on trigger: wave close / B-law or gate-stage add-amend / 30 ratifications; flips to failing after one green wave of reports).
- **S3 · the evidence-store choicepoint** — **OPEN, the owner's call** (the last Core Phase item): fold the law-journal into the vault chain (recommended) vs sidecar with its own chain + signature; the audit-chain persistence point common to both and landable first; pre-analyzed lift-ready in `docs/forge/annex/S3-EVIDENCE-STORE-FORK.md` (D-415 at lift; volume clock measured: 7,145 rows / one full-gate run on the D-413 tree).
