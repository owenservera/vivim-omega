...
- **S3 · the evidence-store choicepoint** — **OPEN, the owner's call — THE ONLY REMAINING CORE PHASE ITEM**: fold the law-journal into the vault chain (recommended) vs sidecar with its own chain + signature; the audit-chain persistence point common to both and landable first; pre-analyzed lift-ready in `docs/forge/annex/S3-EVIDENCE-STORE-FORK.md` (D-416 at lift; volume clock measured: 7,145 rows / one full-gate run on the D-413 tree).
