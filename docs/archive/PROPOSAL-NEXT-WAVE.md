...
> **ARCHIVED 2026-09-18 — superseded; the ask below is already answered.**
> The header said "PROPOSED — awaiting owner greenlight" for D-323 (computation
> routing) and D-324 (provenance linkage); both are RATIFIED in
> `docs/BUILD-DECISIONS.md` — roughly sixty decisions and one head have passed since
> this baseline (`9a1e00f`). Present-day law: `docs/decisions/CURRENT-INVARIANTS.md`.
> Moved per the 2026-09-18 independent recommendation report §5. Content below is
> verbatim as archived — historical build spec, not a live ask.
