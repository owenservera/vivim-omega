// vivim.agent — tokens.ts
// VENDORED from plugins/vivim-law/src/tokens.ts (pure, import-free).
// The capability-handle algebra MUST be byte-identical everywhere it runs:
// law mints/evaluates, the agent plugin attenuates at spawn. A relative
// import across plugin dirs would escape the content-hash trust boundary
// and violate the B2 ports-only law, so the pure functions live here too.
// If tokens.ts changes, this copy MUST be re-vendored (diff the two files).
//
